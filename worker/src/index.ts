...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && url.pathname.startsWith("/avatar/")) {
      ...
      if (!userId) {
        return json(env, errorResponse("unknown", "BAD_REQUEST", "Missing avatar user id."), 400);
      }
    ...
    if (request.method !== "POST" || url.pathname !== "/api") {
      return json(env, errorResponse("unknown", "NOT_FOUND", "Not found."), 404);
    }
    ...
    try {
      ...
      return json(env, response, response.ok ? 200 : 400);
    } catch (error) {
      return json(
        env,
