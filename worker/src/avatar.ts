...
const buildAvatarHeaders = (env: Env, contentType: string | null): Headers => {
...
export async function handleAvatarRequest(request: Request, env: Env, userId: string): Promise<Response> {
  ...
  if (!user?.avatar_url) {
    return new Response("Avatar not found.", {
      status: 404,
      headers: buildAvatarHeaders(env, "text/plain;charset=utf-8"),
    });
  }
  ...
  const response = new Response(upstream.body, {
    status: 200,
    headers: buildAvatarHeaders(env, upstream.headers.get("content-type")),
  });
