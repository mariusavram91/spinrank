...
test.describe("dashboard regression flow", () => {
  ...
  test.beforeEach(async ({ page, request }) => {
    ...
    const sessionUser = await bootstrapTestUser(request, {
      userId: `e2e-dashboard-owner-${timestamp}`,
      displayName: "E2E Dashboard Owner",
    });
    const rival = await bootstrapTestUser(request, {
      userId: `e2e-dashboard-rival-${timestamp}`,
      displayName: "E2E Dashboard Rival",
    });
  ...
  test("refreshes leaderboard, progress, and recent matches after recording a season match", async ({ page }) => {
    const seasonName = `Dashboard Regression Season ${Date.now()}`;
      ...
      if (body?.action === "searchParticipants" && body?.payload?.segmentType === "season") {
        await route.fulfill({
          ...
          body: JSON.stringify({
            ...
            error: null,
            requestId: body?.requestId ?? "dashboard-regression-search",
          }),
        });
        return;
      }
    ...
    await expect(page.getByTestId("leaderboard-list")).toContainText("E2E Dashboard Owner");
    await expect(page.getByTestId("leaderboard-list")).toContainText("1220");
    await expect(page.getByTestId("progress-panel")).toContainText("1220");
    await expect(page.getByTestId("progress-panel")).toContainText(/Wins\s*1/i);
    await expect(page.getByTestId("matches-list")).toContainText(new RegExp(`${rivalDisplayName}|${rivalId}`));
  });
});
